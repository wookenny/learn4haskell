...
-}
subList :: Int -> Int -> [a] -> [a]
subList start end list = if start < 0 then [] else take len (drop start list)
    where
      len = end - start + 1
...
smartReplicate l = concat (map (\el -> replicate el el) l)
...
-}
rotate :: Int -> [a] -> [a]
rotate r xs = if (r < 0) then [] else subList start end (cycle xs)
  where l = length xs
        start = r
        end = r + l - 1
...
rewind :: [a] -> [a]
rewind [] = []
rewind l =  x : rewind xs
    where 
      (x:xs) = rotate (length l -1) l
